...
HRESULT InitWindow(HINSTANCE hInstance, int nCmdShow)
{
    ...
    // Create window
    g_hInst = hInstance;
    RECT rc = {50, 50, 400, 300 };
    AdjustWindowRect(&rc, WS_OVERLAPPEDWINDOW, FALSE);
    g_hWnd = CreateWindow(L"WindowClass", L"lab_1", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
    ...
    // Initialize the translayion matrix
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
        if(pAnnotation) pAnnotation->BeginEvent(L"Start resizing");
        if (g_pSwapChain)
        {
        ...
        if (pAnnotation) pAnnotation->EndEvent();
        break;
    case WM_DESTROY:
        PostQuitMessage(0);
        break;
...
void Render()
{
    pAnnotation->BeginEvent(L"Start Render");
    // Update our time
    static float t = 0.0f;
